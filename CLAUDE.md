...
- **C″ · Cloudflare Pages** (`npm run publicar:cloudflare`, sesión con `npx wrangler login`): proyectos `biocaps-principal` y `biocaps-propuesta` en `*.pages.dev`. Gratis, sin límite de ancho de banda y 500 publicaciones al mes. Sí lee `_headers`.
