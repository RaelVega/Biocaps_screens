...
import { cargarContenidoBiocaps } from './contenido/cargar';
import { ProveedorRecursos, type Recursos } from './estado';
import { Experiencia } from './Experiencia';
import { crearMaquinaBiocaps } from './flujo';
...
async function iniciar(): Promise<Recursos> {
  const { catalogo, manifiesto } = await cargarContenidoBiocaps();
  ...
  const almacen = crearAlmacenFlujo(crearMaquinaBiocaps(catalogo), { avisoMs: AVISO_INACTIVIDAD_MS, reinicioMs: REINICIO_INACTIVIDAD_MS });
  return { almacen, catalogo, manifiesto };
}

/** Arranque: contenido validado y todo precargado antes de la portada. Nunca termina en pantalla blanca. */
